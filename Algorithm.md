...
| ---------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------ | ---------------------------------------------------------- | ----------------------------------------------------------- |
| **序列容器**     | `vector`、`list`、`deque`、`array`、`forward_list`           | 按插入顺序存储，元素位置由插入顺序决定                       | 按插入顺序   | 允许                                                       | `vector`/`deque`/`array` 支持；`list`/`forward_list` 不支持 |
...
| **容器适配器**   | `stack`、`queue`、`priority_queue`                           | 封装其他容器（如 `deque`、`vector`），提供受限接口（如栈、队列、优先级队列逻辑） | -            | 由底层容器决定                                             | 不支持（仅允许访问特定位置元素）                            |
