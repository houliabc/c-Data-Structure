...
![image-20260216162125078](https://houlir2.dpdns.org/2026/02/c441f2a398a2c55ecd58a497319b64fc.png)
...
![image-20260117152016718](https://houlir2.dpdns.org/2026/01/4b5c983786fbcf243ddd2cc4ed5cfefc.png)

![image-20260117152030128](https://houlir2.dpdns.org/2026/01/fc230ac66f2be748c02c8c7231eadfc9.png)

最坏的情况是插入序列是有序的情况，此时树高达到最大，平均查找长度为(n+1)/2
...
**n~h~ = n~h-1~ + n~h-2~ +1**，其中n~0~=0，n~1~=1，n~2~=2

**深度为h的平衡二叉树最少的节点数为满二叉树的情况**

### 红黑树RBT

#### 性质

![image-20260117152816458](https://houlir2.dpdns.org/2026/01/8b7c9ba79db782a3bb328ed8fb2e848a.png)

![image-20260117152829239](https://houlir2.dpdns.org/2026/01/31503528eef9c200e4b85746787d5267.png)

![image-20260119195817346](https://houlir2.dpdns.org/2026/01/c2fec54fdd31cd5dacf7fe220e838fd1.png)

#### 插入

主要看该节点的叔叔节点树红色还是黑色的，若红色，则无需旋转，只需变色即可；若黑色，则旋转后变色

![image-20260117152855356](https://houlir2.dpdns.org/2026/01/954dc9d45f4fa11d40f2766ef500838a.png)

### B树

#### 性质

AVL的扩展，是多路的平衡查找树，且平衡因子都为0

**节点中关键字个数n的范围：[m/2]~上取整~ <= n <= m-1**

![image-20260119191219913](https://houlir2.dpdns.org/2026/01/93ad0573f566fd14120489934fac0645.png)

**B树高度h的范围：[logm(n+1)]~上取整~ <= h <= [log[m/2]~上取整~((n+1)/2) + 1]~上取整~**

由上述也可以推出**B树中关键字个数n的范围：2*[m/2]~上取整~^h-1^ <= n <= [logm(n+1)]~上取整~**

![image-20260119191809973](https://houlir2.dpdns.org/2026/01/35880379b679b2e0363a27ce36614036.png)

#### 插入

当插入到指定位置（叶节点）后，发现关键字个数超出范围后，就要进行分裂的处理，具体的可以**选择[m/2]~上取整~往上插入，而剩下的各放两边，有时候根据题目也可以是下取整**

说白了B树里的插入就是把插入的节点放到叶节点处，而如果此时超出范围后就会分裂，而将分裂出的节点往上移。如果当前层数就只有一层，那么往上移的那一层就会作为根节点。

![image-20260119200019221](https://houlir2.dpdns.org/2026/01/ae3ca7acb4ad9416d2b574e35c1db96a.png)

#### 删除

若当前删除后，不符合原有的关键字个数范围的要求，则有两种方法：

1. **兄弟够借（且不会借后不符合）**：拿前驱的前驱或后驱的后驱放在父节点，而父节点对应的拿下来
2. **兄弟不够借（从父母那借**）：将删除后剩余的关键字与父节点多余的部分进行合并（父母借钱给孩子）

![image-20260119200305859](https://houlir2.dpdns.org/2026/01/17aaa11c14d5ee781ae97c5200198156.png)

### B+树

是分块查找的扩展。B+树是应数据库所需而出现的一种B树的变形树。 

其**关键字n的范围是：[m/2]上取整 <= n <= m**

B+树适合顺序和随机访问

![image-20260119201104630](https://houlir2.dpdns.org/2026/01/5fe35e3c000844954f98e226e9836b88.png)
